...
///   cd examples/cpp/whole_brain_sulc_single_image && mkdir -p build && cd build
///   cmake .. && make
///
/// Then run:
///   ./whole_brain_sulc
...
// Convert an interleaved uint8_t RGB vector (from fs::util::viridis) to
// a vector of scimesh::Color.  NaN vertices are left out entirely:
// their uint8_t RGB values come from the NaN-fallback color (white by
// default), but we detect them by checking the original float data so
// that they remain transparent in the output.
static Mesh convert_fs_mesh(const fs::Mesh &fs_mesh,
    ...
    for (size_t i = 0; i < nv; i++) {
        ...
        if (std::isnan(morph_data[i])) {
            // Medial wall / non-cortex: use white (matching neuroimaging
            // convention where missing data is shown in white).
...
int main(int argc, char **argv) {
    ...
    for (const auto &f : {lh_surf, rh_surf, lh_sulc, rh_sulc,
        ...
        if (!fs::util::file_exists(f)) {
            ...
            std::cerr << "Run from the examples/cpp/whole_brain_sulc_single_image"
