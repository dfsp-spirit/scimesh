...
struct RenderOptions {
    ...
    ///
    /// Rendering is two-sided in that case: a visible back side is lit with the
    /// normal flipped towards the camera, so single-sided geometry (a plane, a
    /// surface patch) looks the same from both sides, and the exactly coplanar
    /// face pairs generated by e.g. `generate_plane()` shade identically no
    /// matter which of the two wins the depth test.
