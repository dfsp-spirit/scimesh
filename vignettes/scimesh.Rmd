...
This means your lighting values directly correspond to what you see on
screen, but GPU renderers like rgl additionally apply an sRGB transfer
function (gamma ~2.2) when writing to the framebuffer, producing higher
perceived contrast.

### Gamma Correction

Pass `gamma = 2.2` to `render_options()` to apply sRGB-like gamma
encoding during rendering.  This brightens midtones and increases
perceived contrast, matching the output of GPU renderers:

```r
render_options(gamma = 2.2)
```

The default is 2.2 (sRGB-like contrast, matching rgl / OpenGL).
Set to 1.0 for no correction.  The formula applied is
\code{value^(1/gamma)}.

You can also apply gamma as post-processing to an existing image:
