...
- Polyline tubes with rotation-minimizing frames (curved edges,
  streamlines)
- Scene line layers: pixel-wide lines without geometry (wireframes,
  graph edges)
- Text labels: anti-aliased TrueType glyphs anchored in world space
  (labels that follow the camera and can be hidden by the geometry) or
  in screen space (titles, panel tags, captions), with optional halos,
  in a bundled font
...
Anti-aliasing can be switched on for a whole session instead of per
render call.
[`render_options()`](https://dfsp-spirit.github.io/scimesh/r/reference/render_options.md)
resolves `aa_samples` from the global option `scimesh.aa_samples`
(default `1`, i.e. no AA) unless you pass it explicitly:
...
This is most noticeable on screen-space lines and points
(e.g. connectome edges, see
...
which get smooth edges instead of a hard staircase pattern. The cost
grows with `aa_samples^2`, both in render time and in memory (the
internal framebuffer is `width * aa_samples` by `height * aa_samples`).

### Text labels (R)

[`text_layer()`](https://dfsp-spirit.github.io/scimesh/r/reference/text_layer.md)
annotates a figure. Labels are billboards: they always face the camera
and keep their size in pixels, and the font ships with the package, so
no system font is needed. Positions are either world space (the label
sticks to the 3D location it annotates, and is hidden when geometry is
in front of it) or screen space (`space = "screen"`, pixels from the top
left corner):
...
Labels can also be measured
([`text_extent()`](https://dfsp-spirit.github.io/scimesh/r/reference/text_extent.md)),
projected from 3D to image pixels
([`world_to_screen()`](https://dfsp-spirit.github.io/scimesh/r/reference/world_to_screen.md)),
rotated (`rotation = 90` reads bottom to top, for a vertical axis
label), and used without any mesh
([`render_text()`](https://dfsp-spirit.github.io/scimesh/r/reference/render_text.md)).
Use `font_file = "path/to/font.ttf"` or the `SCIMESH_FONT` environment
variable to replace the bundled Inter font.
...
system. See
[`CPP_GETTING_STARTED.md`](https://dfsp-spirit.github.io/scimesh/r/CPP_GETTING_STARTED.md)
for details.
